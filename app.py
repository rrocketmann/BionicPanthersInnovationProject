...
def process_video_task(session_id, fps):
    try:
        ...
        frame_count = processor.extract_frames(
            ...
            temp_frames_dir,
            fps,
            progress_callback=lambda p: socketio.emit('progress', {
                'session_id': session_id,
        ...
        # Copy original video
        shutil.copy(video_path, os.path.join(output_dir, f'original_video{os.path.splitext(video_files[0])[1]}'))
        ...
        if poi_file_path and os.path.exists(poi_file_path):
            shutil.copy(poi_file_path, os.path.join(output_dir, os.path.basename(poi_file_path)))
